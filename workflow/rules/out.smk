rule rsync:
    input:
        ...
        globin_read_distribution = expand("{runid}/results/quality_control/bam/{sample}.globin_fraction.tsv", runid=runid, sample=wts_samples),
        mane_read_distribution = expand("{runid}/results/quality_control/bam/{sample}.mane_fraction.tsv", runid=runid, sample=wts_samples),
        rrna_contamination = expand("{runid}/results/quality_control/bam/{sample}.rrna_contamination.tsv", runid=runid, sample=wts_samples),
        rrna_custom_content = expand("{runid}/results/quality_control/bam/rrna_fraction_mqc.json", runid=runid),
        globin_custom_content = expand("{runid}/results/quality_control/bam/globin_fraction_mqc.json", runid=runid),
        mane_custom_content = expand("{runid}/results/quality_control/bam/mane_fraction_mqc.json", runid=runid),
